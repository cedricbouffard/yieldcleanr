...
- **Delay Adjustment**: Automatic optimization of GPS-sensor delay
  compensation
...
yieldcleanr now provides 8 meta-functions that replace the individual
filter functions:
...
**Filter types**: `"header"`, `"gps"`, `"dop"`, `"velocity"`, `"yield"`,
`"moisture"`, `"bounds"`, `"all"`
...
**Anomaly types**: `"overlap"`, `"local_sd"`, `"velocity_jump"`,
`"heading"`, `"position"`, `"all"`

**Actions**: `"filter"` (remove points), `"detect"` (mark only),
`"report"` (summary only)
...
**Threshold types**: `"yield"`, `"velocity"`, `"position"`,
`"moisture"`, `"all"`
...
1.  **Delay Adjustment (Optional)**: Optimize GPS-sensor delay for flow
    and moisture
