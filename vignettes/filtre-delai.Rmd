---
title: "Filtre Delay Adjustment - Position-Coordinate Delay Identification"
subtitle: "Théorie mathématique et implémentation"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Filtre Delay Adjustment - Théorie mathématique}
...
Le Delay Adjustment utilise l'**indice de Moran** pour mesurer l'autocorrélation spatiale du rendement. L'autocorrélation spatiale mesure dans quelle mesure des points proches géographiquement ont des valeurs de rendement similaires.
...
**Objectif du Delay Adjustment** : Maximiser l'indice de Moran en trouvant le délai optimal qui aligne le flux avec la position.
...
delay_adjustment_result <- apply_delay_adjustment(data, delay_range = -25:25, n_iterations = 10)

cat("Délai optimal détecté:", delay_adjustment_result$optimal_delay, "secondes\n")
optimal_rsc <- delay_adjustment_result$rsc_values$mean_score[delay_adjustment_result$rsc_values$delay == delay_adjustment_result$optimal_delay]
...
```{r delay_adjustment-plot, fig.width=12, fig.height=6}
# Créer le dataframe pour le graphique
rsc_data <- delay_adjustment_result$rsc_values

# Graphique des scores Moran
p1 <- ggplot(rsc_data, aes(x = delay, y = mean_score)) +
  geom_line(color = "#2c3e50", size = 1) +
  geom_point(data = rsc_data %>% filter(delay == delay_adjustment_result$optimal_delay),
             aes(x = delay, y = mean_score),
             color = "#e74c3c", size = 4) +
  geom_errorbar(aes(ymin = mean_score - sd_score, ymax = mean_score + sd_score),
                alpha = 0.3, width = 0.5) +
  geom_vline(xintercept = delay_adjustment_result$optimal_delay,
             linetype = "dashed", color = "#e74c3c") +
  annotate("text", x = delay_adjustment_result$optimal_delay + 2,
           y = max(rsc_data$mean_score) - 0.02,
           label = paste("Optimal:", delay_adjustment_result$optimal_delay, "s"),
...
```{r delay_adjustment-comparison, fig.width=14, fig.height=6}
# Données avant correction
data_before <- data

# Données après correction
data_after <- optimize_delays(data, type = "flow", delay = delay_adjustment_result$optimal_delay)
...
```{r delay_adjustment-simulation, fig.width=12, fig.height=8}
...
```{r delay_adjustment-moran-effect, fig.width=10, fig.height=6}
...
Le Delay Adjustment est un outil puissant qui utilise l'autocorrélation spatiale pour détecter et corriger les délais de synchronisation. En maximisant l'indice de Moran, il aligne automatiquement le flux de grain avec la position GPS, éliminant les artefacts visuels et améliorant la qualité des cartes de rendement.
