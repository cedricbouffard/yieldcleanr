...
## Filter Header Status (`filter_header_status`)
...
## Filter GPS Status (`filter_gps_status`)

Élimine les points avec un signal GPS de mauvaise qualité.

**GPSStatus typical values :**
- 4-7 = Good signal
- 1-3 = Poor signal
...
**Typical velocity range :** 0.5 - 10 m/s
...
## Filter Yield Range (`filter_yield_range`)
...
## Filter Moisture Range (`filter_moisture_range`)
...
## Filter Local STD (`filter_local_std`)
...
## Filter Sliding Window (`filter_sliding_window`)
...
## Apply Position Filter (`apply_position_filter`)
...
## Apply Flow Delay (`apply_flow_delay`)
...
## Apply Moisture Delay (`apply_moisture_delay`)
...
## Bitmap Overlap Filter (`apply_overlap_filter`)
...
## Localized SD Filter (`apply_local_sd_filter`)
