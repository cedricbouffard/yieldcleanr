---
title: "test2.csv - Données de Test (Fichier 2)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{test2.csv - Données de Test (Fichier 2)}
...
Ce fichier contient des données de test 2 pour le nettoyage des rendements.
...
```{r raw-data}
file_path <- system.file("extdata", "test2.csv", package = "yieldcleanr")
...
ggplot() +
  ...
  theme_minimal() +
  labs(title = "Données Brutes - test2.csv", subtitle = "Avant nettoyage")
```
...
ggplot() +
  ...
  theme_minimal() +
  labs(title = "Données Nettoyées - test2.csv", subtitle = "Après AYCE")
```
