---
title: "Guide Complet du Nettoyage de Données de Rendement"
subtitle: "Analyse détaillée des 4 échantillons avec yieldcleanr"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Guide Complet - Analyse des 4 Samples}
...
Ce guide présente une analyse complète du nettoyage de données de rendement agricole en utilisant le package **yieldcleanr**. Nous analysons en détail 4 échantillons de données réelles provenant de moissonneuses-batteuses, en montrant l'effet de chaque filtre du pipeline AYCE (Auto Yield Cleaning Engine).
...
delay_adjustment_s1 <- apply_delay_adjustment(data_s1, delay_range = -25:25, n_iterations = 10)
cat("\n🔧 Delay Adjustment - Délai optimal:", delay_adjustment_s1$optimal_delay, "secondes\n")

if (delay_adjustment_s1$optimal_delay != 0) {
  data_s1 <- optimize_delays(data_s1, type = "flow", delay = delay_adjustment_s1$optimal_delay)
...
data_s1 <- filter_data(data_s1, type = "velocity", 
                       min_value = thresholds_s1$velocity$min_velocity, 
                       max_value = thresholds_s1$velocity$max_velocity)
...
```{r sample1-viz, fig.width=12, fig.height=6}
...
delay_adjustment_s2 <- apply_delay_adjustment(data_s2, delay_range = -25:25, n_iterations = 10)
cat("🔧 Delay Adjustment - Délai optimal:", delay_adjustment_s2$optimal_delay, "secondes\n")

if (delay_adjustment_s2$optimal_delay != 0) {
  n_before <- nrow(data_s2)
  data_s2 <- optimize_delays(data_s2, type = "flow", delay = delay_adjustment_s2$optimal_delay)
...
data_s2 <- filter_data(data_s2, type = "velocity", 
                       min_value = thresholds_s2$velocity$min_velocity, 
                       max_value = thresholds_s2$velocity$max_velocity)
...
```{r sample2-viz, fig.width=12, fig.height=6}
# Créer les objets sf pour visualisation
...
delay_adjustment_s3 <- apply_delay_adjustment(data_s3, delay_range = -25:25, n_iterations = 10)
cat("Delay Adjustment - Délai optimal:", delay_adjustment_s3$optimal_delay, "secondes\n")

thresholds_s3 <- calculate_thresholds(data_s3)

data_s3 <- data_s3 %>%
  filter_data(type = "velocity", 
              min_value = thresholds_s3$velocity$min_velocity, 
              max_value = thresholds_s3$velocity$max_velocity) %>%
...
```{r sample3-viz, fig.width=12, fig.height=6}
# Créer les objets sf pour visualisation
...
delay_adjustment_s4 <- apply_delay_adjustment(data_s4, delay_range = -25:25, n_iterations = 10)
cat("Delay Adjustment - Délai optimal:", delay_adjustment_s4$optimal_delay, "secondes\n")

thresholds_s4 <- calculate_thresholds(data_s4)

data_s4 <- data_s4 %>%
  filter_data(type = "velocity", 
              min_value = thresholds_s4$velocity$min_velocity, 
              max_value = thresholds_s4$velocity$max_velocity) %>%
  filter_data(type = "moisture", n_std = 3) %>%
  detect_anomalies(type = "overlap", cellsize = 0.3, overlap_threshold = 0.5) %>%
  detect_anomalies(type = "local_sd", n_swaths = 5, lsd_limit = 3)
...
```{r sample4-viz, fig.width=12, fig.height=6}
# Créer les objets sf pour visualisation
...
```{r all-samples-raw-map, fig.width=14, fig.height=10}
...
```{r all-samples-clean-map, fig.width=14, fig.height=10}
...
```{r density-maps, fig.width=14, fig.height=12}
...
```{r comparison-histograms, fig.width=14, fig.height=10}
...
```{r cv-comparison, fig.width=12, fig.height=6}
...
```{r retention-plot, fig.width=10, fig.height=6}
...
Le filtre Delay Adjustment corrige le délai entre la mesure du flux et la position GPS. Voici les délais optimaux détectés :
...
Le délai de 13 secondes pour le Sample 2 indique un problème important de synchronisation entre le capteur de flux et le GPS.
...
2. **Réduction significative du CV** : Diminution de 24% à 54% selon les échantillons
3. **Détection automatique des problèmes** : Delay Adjustment identifie les délais de synchronisation
4. **Filtrage ciblé** : Chaque filtre cible un type spécifique d'anomalie

Le Sample 2 (maïs) présente le taux de rejet le plus élevé (10.3%) en raison d'un délai de delay adjustment important (13 secondes) et de nombreuses anomalies de vitesse. Les autres échantillons montrent une qualité de données initiale meilleure avec des taux de rétention supérieurs à 94%.

Pour plus de détails sur les filtres individuels, consultez les articles mathématiques dédiés :

- `vignettes("filtre-delai")` - Théorie et mathématiques du Delay Adjustment
